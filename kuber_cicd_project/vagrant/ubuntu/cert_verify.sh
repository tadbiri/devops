...
case $value in
  ...
  1)
    if ! [ "${HOST}" = "master-1" ]
    then
        printf "${FAILED}Must run on master-1 ${NC}\n"
        exit 1
    fi
  ...
  4)
    ...
    check_kubeconfig "kube-proxy" "/var/lib/kube-proxy" "https://${LMASTER_1}:6443"
    check_kubeconfig "kubelet" "/var/lib/kubelet" "https://${MASTER_1}:6443"
    ;;
